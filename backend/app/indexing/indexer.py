...
class Indexer:
    ...
    def mock_parse_website(self, url: str) -> List[Data]:
        ...
        """
        FILE_PATH = os.path.join(os.path.dirname(__file__), 'kizen_resources.csv')
...
if __name__ == "__main__":
    ...
    async def test_indexer():
        ...
        if count > 0:
            # Test search
            results = await indexer.search("kizen", max_results=3)
            print(f"\nSearch results for 'kizen': {len(results)} found")
